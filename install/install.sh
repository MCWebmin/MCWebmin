...
select opt in $OPTIONS; do
	if [ "$opt" = "Apache2" ]; then
		if [ "$DISTRO" = "Debian" ]; then
			DEPENDENCIES="$DEPENDENCIES apache2 php5 php5-cli apache2-mpm-itk"
			WEBSERVER="apache"
		elif [ "$DISTRO" = "RHEL" ]; then
		...
		if [ "$DISTRO" = "Debian" ]; then
			DEPENDENCIES="$DEPENDENCIES lighttpd php5-cli"
			WEBSERVER="lighttpd"
			break
		elif [ "$DISTRO" = "RHEL" ]; then
...
#If Apache2 selected, configure it here.
	...
	select opt in $OPTIONS; do
		if [ "$opt" = "VirtualHost" ]; then
			...
			echo "<VirtualHost *:$APACHEPORT>" > $DOMAIN
